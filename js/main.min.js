const SKILLS = [
  ...
  PROJECTS = [
    ...
    },
    {
      ...
      github_url:
        "https://github.com/MartaZaorska/concentration_calculator_app",
...
class TypeText {
  ...
  type() {
    const e = this.index % this.words.length,
      t = this.words[e];
    let a = this.isDelete ? 100 : 200;
    this.isDelete
      ? (this.text = t.substring(0, this.text.length - 1))
      : (this.text = t.substring(0, this.text.length + 1)),
      (this.element.innerText = this.text),
      this.isDelete || this.text !== t
        ? this.isDelete &&
          "" === this.text &&
          ((this.isDelete = !1), (this.index += 1), (a = 500))
        : ((this.isDelete = !0), (a = 2e3)),
      setTimeout(() => this.type(), a);
  }
}
class UI {
  static scrollUp() {
  ...
  static showMessage(e) {
    ...
    (t.innerText = e),
      t.classList.add("message--active"),
      setTimeout(() => t.classList.remove("message--active"), 3e3);
  }
  static displayData() {
  ...
  static displaySkills() {
    const e = document.querySelector(".skills__content");
    (e.innerHTML = ""),
      SKILLS.forEach((t) => {
        e.innerHTML += `\n        <section class="section__box">${t}</section>\n      `;
      });
  }
  static displayProjects() {
    const e = document.querySelector(".projects__content");
    (e.innerHTML = ""),
      PROJECTS.forEach((t) => {
        const a = t.technologies.reduce(
          (e, t, a, o) => (a === o.length - 1 ? `${e} ${t}` : `${e} ${t} /`),
          ""
        );
        e.innerHTML += `\n        <section class="section__box">\n          <h2 class="project__title">${t.name}</h2>\n          <h4 class="project__subtitle">${a}</h4>\n          <p class="project__text">${t.description}</p>\n          <section class="project__controls">\n            <a rel="noopener noreferrer" href="${t.demo_url}"  target="_blank" class="project__link"><i class="far fa-eye"></i> Podgląd</a>\n            <a rel="noopener noreferrer" href="${t.github_url}"  target="_blank" class="project__link"><i class="fab fa-github"></i> Github</a>\n          </section>\n        </section>\n      `;
      });
  }
  static startHeaderAnimation() {
    const e = document.querySelector(".type"),
      t = JSON.parse(e.getAttribute("data-words"));
    new TypeText(t, e);
  }
  static showScrollUpButton() {
    const e = document.querySelector(".scroll__button");
    window.pageYOffset > 360
      ? e.classList.add("scroll__button--active")
      : e.classList.remove("scroll__button--active");
  }
  static mobileScrollAction() {
    const e = document.querySelectorAll(".section__title");
    UI.showScrollUpButton(),
      e.forEach((e) => {
        if (e.hasAttribute("data-fixed")) {
          const t = parseFloat(e.getAttribute("data-offset"));
          window.pageYOffset <= t &&
            (e.removeAttribute("data-fixed"),
            e.classList.remove("section__title--fixed"));
        } else
          e.getBoundingClientRect().top < 10 &&
            (e.setAttribute("data-offset", `${window.pageYOffset}`),
            e.setAttribute("data-fixed", "true"),
            e.classList.add("section__title--fixed"));
      });
  }
}
const resizeHandler = () => {
  ...
  navbarClickHandler = (e) => {
    ...
    window.scroll({ left: 0, top: a, behavior: "smooth" });
  },
  observerCallback = (e) => {
    e.forEach((e) => {
      e.isIntersecting && e.target.classList.add("section__box--active");
    });
  },
  emailClickHandler = () => {
...
document.addEventListener("DOMContentLoaded", () => {
  const e = document.getElementById("email"),
    t = document.querySelector(".scroll__button"),
    a = document.querySelectorAll(".navbar__item");
  window.addEventListener("resize", resizeHandler),
    a.forEach((e) => e.addEventListener("click", navbarClickHandler)),
    e.addEventListener("click", emailClickHandler),
    t.addEventListener("click", UI.scrollUp),
    ...
    (async () => {
      await UI.displayData(),
        await (() => {
          const e = document.querySelectorAll(".section__box");
          if (
            "IntersectionObserver" in window &&
            "IntersectionObserverEntry" in window &&
            "isIntersecting" in window.IntersectionObserverEntry.prototype
          ) {
            let t = new IntersectionObserver(observerCallback, {
              threshold: 0.4,
            });
            e.forEach((e) => t.observe(e));
          } else e.forEach((e) => e.classList.add("section__box--active"));
        })(),
        resizeHandler();
    })();
});
